...
data "aws_iam_policy_document" "recorder_publish_policy" {
  ...
  statement {
    ...
    resources = ["arn:aws-us-gov:kms:*:${data.aws_caller_identity.current.account_id}:key/${var.config_sns_topic_kms_master_key_id != null ? var.config_sns_topic_kms_master_key_id : ""}"]
  }
}
...
resource "aws_iam_role_policy_attachment" "recorder_read_policy" {
  ...
  role       = one(aws_iam_role.recorder[*].id)
  policy_arn = "arn:aws-us-gov:iam::aws-us-gov:policy/service-role/AWS_ConfigRole"
}
...
module "config_baseline_us-gov-west-1" {
  count  = var.config_baseline_enabled && contains(var.target_regions, "us-gov-west-1") ? 1 : 0
  ...
  include_global_resource_types = var.config_global_resources_all_regions ? true : var.region == "us-gov-west-1"
...
resource "aws_iam_role_policy_attachment" "config_organization" {
  ...
  policy_arn = "arn:aws-us-gov:iam::aws-us-gov:policy/service-role/AWSConfigRoleForOrganizations"
}
